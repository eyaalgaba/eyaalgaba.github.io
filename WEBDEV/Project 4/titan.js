...
const loadTitan = async () =>{
    const titanResponse = await fetch (`https://api.attackontitanapi.com/titans/`);
    const titan = await titanResponse.json();

    for (var i = 1; i <= titan.info.count; i++){
        const characterResponse = await fetch (`https://api.attackontitanapi.com/titans/` + i);
